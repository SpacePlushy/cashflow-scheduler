...
def _build_model(plan: Plan):
    ...
    max_day = max(SHIFT_NET_CENTS.values())
    prefix_net = [model.NewIntVar(0, max_day * (t + 1), f"pref_{t}") for t in range(30)]
...
def enumerate_ties(plan: Plan, limit: int = 5) -> List[CPSATSolution]:
    ...
    w, b2b, absd, statuses = _solve_sequential_lex(model_opt, obj_parts, solver_opt)
